...
int mm,dd,yy;//mm=month, dd=day,yy=year
...
void main(void)
{
    ...
    getch();  //For execution of program//
    ...
    fflush(stdin);  //Output buffer fflush//
    getch();
    system("CLS");
...
void addrec(int j)
{
    ...
    if (j==1)
    {
        while(c=='y'||c=='Y')
        {
            ...
            stud.dt.m=cdat;//month of date type of student structure
            ff=stud.c/10.0;
            ...
            fs=fopen("student.txt","ab+");//opening a binary file in append mode
            fwrite(&stud,sizeof(stud),1,fs);
            fclose(fs);
            printf("\n\nDo you want to continue with the process(press y or Y");
            fflush(stdin);
            c=getch();  //char type data input execution in c//
    ...
    if (j==1)
    {
        while(c=='y'||c=='Y')
        {
            ...
            if (choice==1)
            {
                ...
                {
                    if (strcmpi(name,stud.n)==0)//string compare capital & small
                    {
                        a=0;
                        printf("\nname = %s",stud.n);
                        printf("\nclass = %d",stud.c);
            ...
            if (choice==1)
            {
                ...
                while(fread(&stud,sizeof(stud),1,fs)==1)
                {
                    ...
                    if (strcmpi(name,stud.n)==0)
                    {
                        ...
                        fflush(stdin);
                        scanf("%d",&stud.r);
                        fseek(fs,-sizeof(stud),SEEK_CUR);  //file position of string,current position of file//
                        fwrite(&stud,sizeof(stud),1,fs);
                        fclose(fs);
                    }
                }
...
void salary(int mm)
{
    ...
    while(c=='y'||c=='Y')
    {
        ...
        {
            if(strcmpi(tech.n,name)==0 )//name entered is compared to the existing name in file
            {
                float lsal;
                a=0;
